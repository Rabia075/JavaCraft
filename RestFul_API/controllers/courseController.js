...
exports.createCourse = async (req, res) => {
  try {
    const { course_name, course_code, credit_hours, department_id } = req.body;
    ...
    const result = await pool.query(
      ...
      [course_name, course_code, credit_hours, department_id]
    );
...
// ==========================
// Get All Courses (or by query filter)
exports.getAllCourses = async (req, res) => {
  try {
    ...
    const query = filters.length
      ? `SELECT * FROM Courses WHERE ${filters.join(' AND ')}`
      : `SELECT * FROM Courses`;
...
// ==========================
// Get Specific Column by Filter Column
...
exports.getColumnByFilter = async (req, res) => {
  ...
  const validColumns = ['course_id', 'course_name', 'course_code', 'credit_hours', 'department_id'];
...
// ==========================
// Update Course by ID (with record check)
exports.updateCourse = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE Courses
       SET course_name = $1, course_code = $2, credit_hours = $3, department_id = $4
       WHERE course_id = $5 RETURNING *`,
      [course_name, course_code, credit_hours, department_id, id]
    );
...
// ==========================
// Delete Course by ID (with check)
